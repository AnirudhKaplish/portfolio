...
import { X } from 'lucide-react';
  ...
  const TerminalIntro: React.FC<TerminalIntroProps> = ({ onComplete, onAdminMode, onMinimize, instant = false, isResizing = false }) => {
    ...
    // Dynamic Age Calculation (From June 23, 2005)
    const birthDate = new Date('2005-06-23');
    const currentDate = new Date();
    const ageInYears = Math.floor((currentDate.getTime() - birthDate.getTime()) / (1000 * 60 * 60 * 24 * 365.25));
  
    // Dynamic Experience Calculation (From July 1, 2025)
    const expStartDate = new Date('2025-07-01');
    ...
    const bootSequence = [
      ...
      { text: "> hello - i'm Anirudh", color: "text-white font-bold" },
      { text: "> UI/UX Designer | Data Scientist | Solutions Architect | FullStack Developer", color: "text-gray-300" },
